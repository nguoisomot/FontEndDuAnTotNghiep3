...
export default class LineDemo extends Component {
  ...
  showChart = () => {
    ...
    this.state.dataMonthAuth.map((value, index) => {
      monthArray.push(value.tenSanPham);
      dataArray.push(value.soLuong)
      console.log(this.state.dataDayAuth)
    })
    ...
    this.state.dataDayAuth.map((value, index) => {
      dayArray.push(value.tenSanPham);
      dataGiaDay.push(value.soLuong)
    })
  ...
  componentDidMount() {
    ...
    let arrayData = [];
    var dataFinal = null;
    AuthService.sanPhamBanChayTuDauNamDenNay((date.getYear() - 100 + 2000)).then(res =>
       {this.setState({ dataMonthAuth: res.data.msg })
    console.log(res.data.msg)
       })
    AuthService.topSanPhamBanChayThangNay((date.getYear() - 100 + 2000) +"-"+(date.getMonth()+1)).then(res => this.setState({ dataDayAuth: res.data.msg }))
