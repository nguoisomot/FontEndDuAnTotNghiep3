...
export default class SanPhamBanChay extends Component{
  ...
  componentDidMount(){
    ...
    AuthService.sanPhamBanChayHomNay(hom_nay_1, hom_nay_2).then(res => {
      try {
        this.setState({ sp_ban_chay_hom_nay: res.data.msg[0].tenSanPham + " x " + res.data.msg[0].soLuong })
      } catch (error) {
    ...
    AuthService.sanPhamBanChayThangNay((date_thang.getYear() - 100 + 2000) + "-" + (date_thang.getMonth() + 1)).then(res => {
      try {
        this.setState({ sp_ban_chay_thang_nay: res.data.msg[0].tenSanPham + " x " + res.data.msg[0].soLuong});
        console.log(res.data.msg[0].tenSanPham)
      } catch (error) {
    ...
    AuthService.sanPhamBanChayNamNay((date_nam.getYear() - 100 + 2000), JSON.parse(localStorage.getItem('shop'))._id).then(res => {
      try {
        this.setState({ ban_chay_nam_nay: res.data.msg[0].tenSanPham + " x " + res.data.msg[0].soLuong})
      } catch (error) {
