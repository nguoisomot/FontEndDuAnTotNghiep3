...
export default class ThongKe extends Component{
  ...
  componentDidMount(){
    ...
    AuthService.thongKeTheoNgay(hom_nay_1,hom_nay_2, JSON.parse(localStorage.getItem('shop'))._id).then(res =>{
      try {     
        this.setState({ doanh_thu_hom_nay: res.data.msg[0].tongTien })   
      } catch (error) {
    ...
    AuthService.thongKeTheoThang((date_thang.getYear() - 100 + 2000)+"-"+(date_thang.getMonth()+1), JSON.parse(localStorage.getItem('shop'))._id).then(res=>{
      try {
        this.setState({doanh_thu_thang_nay:res.data.msg[0].tongTien});
        console.log(res)
      } catch (error) {
    ...
    AuthService.thongKeTheoNam((date_nam.getYear() - 100 + 2000), JSON.parse(localStorage.getItem('shop'))._id).then(res=>{
      try {
        this.setState({doanh_thu_nam_nay:res.data.msg[0].tongTien})
      } catch (error) {
